...
class MMHT(BaseTracker):
    ...
    def initialize(self, image, event_img, info: dict) -> dict:
        ...
        event_im = [numpy_to_torch(event_img[0]), numpy_to_torch(event_img[1]), numpy_to_torch(event_img[2])]
        ...
        init_backbone_feat = self.generate_init_samples(im, event_im)

        # Initialize classifier
        losses = self.init_classifier(init_backbone_feat)
        ...
        out = {'time': time.time() - tic,
               'cls': [loss.item() for loss in losses]}
        return out


    def track(self, image, event_img, info: dict = None) -> dict:
        self.debug_info = {}
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                      ...
                                                                      self.img_sample_sz)
        backbone_feat = self.extract_backbone_event_features(backbone_feat, event_im, self.get_centered_sample_pos(),
                                                                      self.target_scale * self.params.scale_factors,
                                                                      self.img_sample_sz)
        ...
        cam = {'search':self.search_area_box,'score': score_map}
    ...
    def extract_backbone_event_features(self, backbone_feat, event_im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        event_patches = []
        for event in event_im:
            temp_patch, _ =  sample_patch_multiscale(event, pos, scales, sz,
                                                           mode=self.params.get('border_mode', 'replicate'),
                                                           max_scale_change=self.params.get('patch_max_scale_change', None))
            temp_patch = temp_patch / 255
            temp_patch -= torch.Tensor((0.485, 0.456, 0.406)).view(1, -1, 1, 1)
            temp_patch /= torch.Tensor((0.229, 0.224, 0.225)).view(1, -1, 1, 1)
            temp_patch = temp_patch.cuda()
            event_patches.append(temp_patch)

        with torch.no_grad():
            output_event_features = self.net.extract_backbone_features(im=torch.stack([event_patches[0].reshape(-1, *event_patches[0].shape[-3:]),
                                                                                       event_patches[1].reshape(-1, *event_patches[1].shape[-3:]),
                                                                                       event_patches[2].reshape(-1, *event_patches[2].shape[-3:])], axis=1), modal='dvs')
            backbone_feat = self.net.fusion_block(backbone_feat['layer2'], output_event_features['layer2'], backbone_feat['layer3'], output_event_features['layer3'])
    ...
    def generate_init_samples(self, im: torch.Tensor, event_im) -> TensorList:
        ...
        event_im_patches = []
        for event in event_im:
            temp_patch = sample_patch_transformed(event, self.init_sample_pos, self.init_sample_scale, aug_expansion_sz, self.transforms)
            temp_patch = temp_patch / 255
            temp_patch -= torch.Tensor((0.485, 0.456, 0.406)).view(1, -1, 1, 1)
            temp_patch /= torch.Tensor((0.229, 0.224, 0.225)).view(1, -1, 1, 1)
            temp_patch = temp_patch.cuda()
            event_im_patches.append(temp_patch)

        # Extract initial backbone features
        with torch.no_grad():
            init_backbone_feat_ann = self.net.extract_backbone(im_patches)
            init_backbone_feat_snn = self.net.extract_backbone_features(torch.stack([event_im_patches[0].reshape(-1, *event_im_patches[0].shape[-3:]),
                                                                                     event_im_patches[1].reshape(-1, *event_im_patches[1].shape[-3:]),
                                                                                     event_im_patches[2].reshape(-1, *event_im_patches[2].shape[-3:])], axis=1), modal='dvs')

            init_backbone_feat=self.net.fusion_block(init_backbone_feat_ann['layer2'], init_backbone_feat_snn['layer2'], init_backbone_feat_ann['layer3'], init_backbone_feat_snn['layer3'])
    ...
    def visdom_draw_tracking_save(self, image, box, segmentation=None):
        if hasattr(self, 'search_area_box'):
            self.visdom.register((image, box, self.search_area_box), 'Tracking', 1, 'Tracking')
        else:
            self.visdom.register((image, box), 'Tracking', 1, 'Tracking')

    def visdom_draw_tracking(self, image, box, gt, segmentation=None):
        if hasattr(self, 'search_area_box'):
            self.visdom.register((image, box, self.search_area_box, gt), 'Tracking', 1, 'Tracking')
        else:
            self.visdom.register((image, box), 'Tracking', 1, 'Tracking')
